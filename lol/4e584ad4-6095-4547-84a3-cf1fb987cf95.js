...
const status = ["BadGuyMusic","Credit By mahyar",`${client.guilds.cache.size}Servers`,"1play","1help",]

let rstatus = status[Math.floor(Math.random() * status.length)]

client.user.setPresence({
        status: 'command music=1p',
        activity: {
            name: `${rstatus}`,
            type: 'PLAYING',
        }
...
client.on("message", message => {
  ...
  const FieldsEmbed = new Pagein.FieldsEmbed()
   .setArray([
     {word:"***دستورات کلی***"},
     {word:"\u200B"},
     {word:"**1helpm** لیست کامندهای موزیک"},
     {word:"\u200B"},
     {word:'**1hello** سلام کردن به بات'},
     {word:"\u200B"},
     {word:'**1games** یه لیست از گیم های دیسکورد'},
     {word:"\u200B"},
     {word:'**1post <text>** پست کردن یک امبد با متن خودتان'},
     {word:"\u200B"},
     {word:'**1info** اطلاعات سرور'},
     {word:"\u200B"},
